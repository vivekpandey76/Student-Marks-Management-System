...
<div id="mySidebar" class="sidebar">
	<a href="javascript:void(0)" class="closebtn" onclick="closeNav()">×</a>
 		<br>
 			...
 			<div><a style="color: white" href="iat2.php" ><span class="glyphicon glyphicon-text-width "></span> IAT-2</a>
 			</div>
 ...
 <button class="openbtn sidebar-heading" onclick="openNav()">☰</button> 
 	...
 	if(isset($_POST['reset'])){
 		$sql = "UPDATE `iat2` SET `1a` = 'default', `1b` = 'default', `2a` = 'default', `2b` = 'default', `3` = 'default', `4` = 'default', `final` = 'default' ";
...
<?php 
	...
	if(isset($_POST['save_changes'])){
		...
		foreach ($_POST['id'] as $id) {
			...
			$update = "UPDATE `iat2` SET `1a` = '$one_a', `1b` = '$one_b', `2a` = '$two_a', `2b` = '$two_b', `3` = '$three', `4` = '$four', `final` = '$final' WHERE `iat2`.`id` = $id " ;
			mysqli_query($conn , $update);
		}
	}

	$sql = "SELECT * FROM iat2";
